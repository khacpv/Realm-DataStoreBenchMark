...
public class TestSQLite extends TestDataStore {
    ...
    private void addRows() {
        SQLiteStatement stmt = db.compileStatement("INSERT INTO " + dbHelper.TABLE_NAME + " VALUES(?1, ?2, ?3, ?4)");
        db.beginTransaction();
    ...
    private void verify() {
        Cursor cursor = db.rawQuery(String.format("SELECT COUNT(*) AS count FROM %s", dbHelper.TABLE_NAME), null);
        cursor.moveToFirst();
    ...
    private void deleteRows() {
        db.beginTransaction();
        db.execSQL(String.format("DELETE FROM %s", dbHelper.TABLE_NAME));
        db.setTransactionSuccessful();
        db.endTransaction();
    }
    ...
    public void testSum() {
        ...
        Benchmark benchmark = new Benchmark() {
            ...
            @Override
            public void run() {
                Cursor cursor = db.rawQuery(String.format("SELECT SUM(age) AS sum FROM %s", dbHelper.TABLE_NAME), null);
                cursor.moveToFirst();
    ...
    public void testSimpleWrite() {
        ...
        Benchmark benchmark = new Benchmark() {
            ...
            public void setUp() {
                addTable();
                stmt = db.compileStatement("INSERT INTO " + dbHelper.TABLE_NAME + " VALUES(?1, ?2, ?3, ?4)");
                i = 0;
            }
